...
import { ProviderPOSPanel } from './components/ProviderPOSPanel';
...
export default function App() {
  ...
  }, [activeWorkerId, activeProviderId, payrollFilterMonth, addNotification]);
  ...
  // ─── ADMIN: PROCESAR NÓMINA MES ───────────────────────────────────────────────
  const handleProcessPayrollDeductions = async () => {
    const targets = installments.filter(i => (i.fecha_cobro || '').slice(0, 7) === payrollFilterMonth && i.estatus === 'Pendiente');
    if (targets.length === 0) { addNotification('warning', 'Sin Cuotas', 'No hay cuotas pendientes para este mes.'); return; }
    ...
    try {
      ...
      addNotification('success', 'Nómina Procesada', `${targets.length} cuotas descontadas exitosamente para el mes.`);
      await fetchData();
    } catch (err: any) { addNotification('error', 'Error de Nómina', err.message); }
    finally { setIsProcessingPayroll(false); }
  };

  // ─── ADMIN: EXPORTAR CSV MES ─────────────────────────────────────────────────
  const handleExportPayrollCSV = () => {
    const targets = installments.filter(i => (i.fecha_cobro || '').slice(0, 7) === payrollFilterMonth && i.estatus === 'Pendiente');
    if (targets.length === 0) { addNotification('warning', 'Sin Datos', 'No hay cuotas pendientes para exportar en este mes.'); return; }
    ...
    const a = document.createElement('a'); a.href = url; a.download = `Nomina_CrediCRC_${payrollFilterMonth}.csv`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    addNotification('success', 'CSV Exportado', `Reporte de nómina ${payrollFilterMonth} descargado.`);
  };
  ...
  // ─── RESUMEN FINANCIERO MENSUAL (computado según mes seleccionado) ────────
  const _now = new Date();
  const _curMonth = `${_now.getFullYear()}-${String(_now.getMonth() + 1).padStart(2, '0')}`;
  const activeMonthFilter = payrollFilterMonth || _curMonth;
  const monthlyTx = transactions.filter(t => (t.fecha_transaccion || '').slice(0, 7) === activeMonthFilter);
                    ...
                    {currentProvider ? (() => {
                      const posSlotNode = (
                        <ProviderPOSPanel
                          ...
                          scannedWorkerInfo={scannedWorkerInfo}
                          posAmount={posAmount}
                          setPosAmount={setPosAmount}
                          aplicaInicial={aplicaInicial}
                          setAplicaInicial={setAplicaInicial}
                          posDays={posDays}
                          setPosDays={setPosDays}
                          isValidating={isValidating}
                          scanRetrying={scanRetrying}
                          scanRetryCount={scanRetryCount}
                          validationResult={validationResult}
                          setValidationResult={setValidationResult}
                          inicialConfirmada={inicialConfirmada}
                          setInicialConfirmada={setInicialConfirmada}
                          ...
                          onCancel={() => {
                            ...
                            setValidationResult(null);
                            setPosAmount('');
                            setInicialConfirmada(false);
                          }}
                          onCalculateAmount={handleCalculateAmount}
                          onProcessPurchase={() => handleProcessPurchaseDebounced()}
                        />
                      );
                        ...
                        {adminTab === 'payroll' && (() => {
                          ...
                          <div className="mt-6 space-y-3">
                            ...
                            {monthlySummaryRows.length === 0 ? (
                              <div className="border-2 border-dashed border-slate-200 rounded-xl py-8 text-center text-slate-400 text-xs font-semibold">Sin transacciones registradas para este mes.</div>
                            ) : (
